...
if __name__ == "__main__":
    ...
    out_dir = "/media/harrison/the_ark/blender_renders/topological_glow_ocean/"
    pattern = ".png"
    ...
    for i in rng:
        ...
        bpy.ops.mesh.primitive_plane_add(size=999, enter_editmode=True, align='WORLD', location=(0, 0, 0), scale=(1, 1, 1))
        ...
        bpy.context.object.modifiers["Ocean"].wave_scale = 111.1
        bpy.context.object.modifiers["Ocean"].resolution = 7
        bpy.context.object.modifiers["Ocean"].size = 53
        bpy.context.object.modifiers["Ocean"].spatial_size = 43
        bpy.context.object.modifiers["Ocean"].random_seed = 69
        bpy.context.object.modifiers["Ocean"].depth = 111.01
        bpy.context.object.modifiers["Ocean"].wave_scale_min = 0
        bpy.context.object.modifiers["Ocean"].wave_alignment = 0.157534
        bpy.context.object.modifiers["Ocean"].damping = 0.5
        bpy.context.object.modifiers["Ocean"].choppiness = 0
        bpy.context.object.modifiers["Ocean"].wind_velocity = 7.3
        bpy.context.object.modifiers["Ocean"].resolution = 34
        wave_time = 18.2+i/20
        bpy.context.object.modifiers["Ocean"].time = wave_time
        ##############END OCEAN
        ...
        for k in rng:
            k = int(k)
            print(k)
            bpy.ops.mesh.primitive_plane_add(size=999, enter_editmode=False, align='WORLD', location=(0, 0, .75*k), scale=(1, 1, 1))
            ...
            if (bpy.context.object.type == "CURVE"):
                ...
                mat3 = newShader("contour"+str(k),"emission", r_arr[k],g_arr[k],b_arr[k], 1)
                bpy.context.active_object.data.materials.append(mat3)
                trigger = 1
            elif (bpy.context.object.type != "CURVE" and trigger == 1):
                print('Plane did not form curve. Ending loop early')
                bpy.ops.object.delete()
        ...
        cam_obj1.location = (707.83,252.79,410.35)
        cam_obj1.rotation_euler = rotation=(60.3*3.1415/180, 0.0, 107*3.1415/180)
